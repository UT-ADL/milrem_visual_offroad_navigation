...
class Mask:
    ...
    def create_masked_map(self, map_tensor, trajectory):
        start_mask = np.expand_dims(self.create_mask(map_tensor.shape[1:], trajectory[[0]]), axis=0)
        end_mask = np.expand_dims(self.create_mask(map_tensor.shape[1:], trajectory[[-1]]), axis=0)
    ...
    def create_mask(self, mask_shape, trajectory):
        ...
        if self.mask_type == 'gaussian':
            ...
            mask = create_gaussian_mask(mask, self.kernel_size, self.sigma)
            mask = create_distance_mask(mask, limit_trajectory(trajectory, mask_shape))
...
def limit_trajectory(trajectory, shape):
    ...
    for i in range(len(trajectory)):
        ...
        if goal_x >= shape[0]:
            trajectory[i][0] = shape[0] - 1

        if goal_x < 0:
            trajectory[i][0] = 0

        if goal_y >= shape[1]:
            trajectory[i][1] = shape[1] - 1
...
def create_distance_mask(trajectory_mask, trajectory):
    ...
    distance_mask = (distance_mask - min) / (max - min)
