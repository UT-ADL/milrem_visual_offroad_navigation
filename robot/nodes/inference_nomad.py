...
class Visualizer:
    ...
    def timer_callback(self, event=None):
        ...
        local_model_out = self.local_session.predict(obs_tensor=obs_img, goal_tensor=self.goal_img_preprocessed)

        inference_end = time.time()
        print(f"Time taken to perform inference: {inference_end - inference_start} seconds")

        distance_predictions = local_model_out[0]
        waypoint_predictions = local_model_out[1][:, :5, :2]

        # Check if final goal is reached
        if distance_predictions[-1] <= self.goal_conditioning_threshold:
        ...
        candidate_waypoints = waypoint_predictions[:, -1, :]
        ...
        goal_x, goal_y = waypoint_predictions[best_waypoint, -1]
        print(f"Goal:\n{goal_x, goal_y}")
        print(f"best waypoint: {waypoint_predictions[best_waypoint]}")
        ...
        # Draw the waypoints
        for i in range(len(waypoint_predictions)):
            visualize_waypoints(rectified_img, waypoint_predictions[i], 
                                waypoint_predictions.shape[1], 
                                self.tf_from_cam_frame_to_optical_frame, 
                                self.camera_model, 
                                color=TRAJ_COLORS[1], # RED 
                                radius=5)
        visualize_waypoints(rectified_img, 
                            waypoint_predictions[best_waypoint], 
                            waypoint_predictions.shape[1], 
                            self.tf_from_cam_frame_to_optical_frame, 
                            self.camera_model, 
                            color=TRAJ_COLORS[0], # BLUE 
                            radius=5)
        ...
        self.draw_info_overlay(frame=rectified_img,
                                predicted_distance=distance_predictions[-1],
                                goal_x=goal_x,
                                goal_y=goal_y,
                                v=v,
                                w=w)
...
if __name__ == "__main__":
    rospy.init_node("NoMaD_Inference_Visualization", log_level=rospy.INFO)
    node = Visualizer()    
    rospy.spin()
