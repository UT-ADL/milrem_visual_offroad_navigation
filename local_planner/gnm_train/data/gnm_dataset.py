...
class GNMDataModule(LightningDataModule):

    def __init__(self, config):
        super().__init__()
        self.config = config
        self.train_loader, self.val_loader = self.initalize_loaders()
    ...
    def initalize_loaders(self):
        ...
        aspect_ratio = self.config["image_size"][0] / self.config["image_size"][1]
        ...
        for dataset_name in self.config["datasets"]:
            ...
            for data_split_type in ["train", "test"]:
                if data_split_type in data_config:
                    for output_type in ["action", "distance", "pairwise"]:  # TODO: add pairwise
                        ...
                        if output_type == "pairwise":
                            dataset = PairwiseDistanceDataset(
                                ...
                                close_far_threshold=self.config["close_far_threshold"],
                            ...
                            dataset = GNM_Dataset(
                                ...
                                aspect_ratio=aspect_ratio,
                                waypoint_spacing=data_config["waypoint_spacing"],
                                min_dist_cat=self.config[output_type]["min_dist_cat"],
                                max_dist_cat=self.config[output_type]["max_dist_cat"],
                                negative_mining=data_config["negative_mining"],
                        ...
                        if data_split_type == "train":
                            ...
                            if dataset_type not in test_datasets:
                                test_datasets[dataset_type] = {}
                            test_datasets[dataset_type][output_type] = dataset
        ...
        iterables = {
            "distance": train_dist_loader,
            "action": train_action_loader
        }

        train_loader = CombinedLoader(iterables, 'min_size')

        test_loaders = {} # TODO: validation loader would be correct
        for dataset_type in test_datasets:
            test_distance_loader = DataLoader(
                test_datasets[dataset_type]["distance"],
                batch_size=self.config["eval_batch_size"],
                shuffle=True, # TODO: why true?
                num_workers=int(self.config["num_workers"]),
                drop_last=True,
                pin_memory=True
            )

            test_action_loader = DataLoader(
                test_datasets[dataset_type]["action"],
                batch_size=self.config["eval_batch_size"],
                shuffle=True, # TODO: why true?
                num_workers=int(self.config["num_workers"]),
                drop_last=True,
            ...
            #     test_datasets[dataset_type]["pairwise"],
            #     batch_size=self.config["eval_batch_size"],
            #     shuffle=True,
            #     num_workers=self.config["num_workers"],
            #     drop_last=True,
            ...
            # )
            # print(f"{test_pairwise_loader}_test_pairwise_loader", len(test_pairwise_loader))


            test_loaders[dataset_type] = CombinedLoader({
                "distance": test_distance_loader,
                "action": test_action_loader,
                #"pairwise": test_pairwise_loader # TODO: why one is zero length?
            }, 'min_size')
