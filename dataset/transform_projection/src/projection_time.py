...
rospy.init_node("projection_time")
fixed_frame = "map"
...
tf_buffer = tf2_ros.Buffer(rospy.Duration(int(max_len * 1.2)))
...
def filter_callback(image_msg, pose_msg):
    ...
    last_pose_stamp = message_queue[-1][-1].header.stamp
    new_pose_stamp = pose_msg.header.stamp

    if (new_pose_stamp - last_pose_stamp) >= rospy.Duration(1):
        message_queue.append((image_msg, pose_msg))
